...
#define MAX_RECONNECTION_TRIES          (5)
...
static void event_handler(void* event_handler_arg, esp_event_base_t event_base, int32_t event_id, void* event_data)
{
    ...
    case WIFI_EVENT_STA_DISCONNECTED:
    {
        ...
        ESP_LOGE(TAG, "Disconnection reason: %s", get_wifi_error(wifi_event_sta_disconnected->reason));
        if (wifi_event_sta_disconnected->reason != WIFI_REASON_ASSOC_LEAVE) {
            /** Try to reconnect if user has not triggered disconnection */
            if (disconnection_err_count++ < MAX_RECONNECTION_TRIES) {
                esp_wifi_connect();
                break;
            }
        } 
    ...
    case WIFI_EVENT_AP_STACONNECTED:
    {
        ...
        ESP_LOGI(TAG, "Device %s connected", wifi_event_ap_staconnected->mac);
        break;
    }

    case WIFI_EVENT_AP_STADISCONNECTED:
        wifi_event_ap_stadisconnected_t *wifi_event_ap_stadisconnected = event_data;
        ESP_LOGI(TAG, "Device %s connected, reason %d", 
                                                        wifi_event_ap_stadisconnected->mac,
                                                        wifi_event_ap_stadisconnected->reason);
        break;
